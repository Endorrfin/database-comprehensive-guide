      ...
      blocks: [
        {
          ...
          md: {
            ...
            uk: "**Wide-column store** — це не таблиця зі стовпчиками, а модель, де кожен рядок може мати інший набір стовпчиків, а *partition* обʼєднує багато рядків під єдиним partition key. Apache **Cassandra** (найновіша стабільна: **5.0.7, випущена 2026-03-23**) описує себе як *partitioned wide-column storage model*.\n\nКожна таблиця має **primary key** з двох частин:\n\n- **Partition key** (обовʼязково): хешується для визначення того, які вузли зберігають дані. Всі рядки з однаковим partition key знаходяться на одному наборі реплік і можуть бути отримані одним I/O-sweep. Запит без partition key повинен сканувати кожну partition — уникайте цього.\n- **Clustering key(s)** (опційно): визначають фізичний порядок сортування рядків *усередині* partition. Уявіть partition як міні відсортовану таблицю. В таблиці `sensor_data` partition key може бути `(device_id)`, а clustering key — `timestamp DESC`, що безкоштовно дає найновіші зчитування першими.\n\nТермін *wide row* походить від того, що одна partition може містити мільйони пар clustering-key/value — наприклад, кожне зчитування сенсора для одного пристрою. Це канонічний Cassandra time-series патерн.",
          },
        },
        {
          kind: 'figure',
        ...
        },
        {
          ...
          md: {
            ...
            uk: "Cassandra має мʼяке обмеження розміру partition у 2 ГБ, але на практиці partition вище ~100 МБ викликають проблеми з compaction і repair. Якщо ваш wide row зростає необмежено (наприклад, всі зчитування пристрою назавжди), додайте виміру **bucketing** до partition key — наприклад `(device_id, year_month)` — щоб кожна partition залишалась обмеженою.",
          },
        },
      ],
    },
      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            [
              { en: 'STCS', uk: 'STCS' },
              { en: 'Merge SSTables of similar size tiers', uk: 'Обʼєднує SSTables подібного розміру' },
