...
FROM pg_stat_wal_receiver;`,
  ...
  keyPoints: [
    ...
    },
    {
      ...
      uk: 'pg_stat_replication на primary показує sent_lsn, write_lsn, flush_lsn, replay_lsn та їхні інтервали lag — перший інструмент для вимірювання здоров\'я реплікації.',
    },
    {
  ...
  pitfalls: [
    ...
    },
    {
      title: {
        ...
        uk: 'Маршрутизація записів на standby після failover (застаріла конфіг пула з\'єднань)',
      },
      body: {
        ...
        uk: "Застосунки, що кешують адресу хоста primary (або використовують застарілу конфігурацію пула з'єднань), надсилатимуть записи на старий primary — тепер standby — і отримуватимуть помилки або тихий режим read-only. Використовуйте **virtual IP** (HAProxy, pgBouncer з перевіркою здоров'я або хмарний балансувальник навантаження) перед Patroni, щоб endpoint з'єднання залишався стабільним під час failover. Patroni інтегрується з HAProxy і etcd-based dynamic DNS з коробки.",
      },
    },
  ],
  interview: [
