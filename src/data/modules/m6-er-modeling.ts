      ...
      blocks: [
        {
          ...
          md: {
            ...
            uk: "Дизайн схеми відбувається на **трьох рівнях**, кожен уточнює попередній. **Conceptual** модель — це ER-діаграма: entities та relationships, жодного движка — це те, що ви малюєте на дошці зі стейкхолдерами. **Logical** модель перетворює це на **relations**: таблиці, колонки, primary і foreign keys, нормалізовані (M7) — усе ще без прив'язки до движка, без типів даних. **Physical** модель — це реальний PostgreSQL: конкретні типи (M9), indexes (M13–M14), partitions, storage. Дисципліна — **не перестрибувати рівні**: усунути відсутній relationship безкоштовно на дошці й дорого, коли це вже пʼятдесят таблиць і продакшн-міграція.",
          },
        },
        {
          kind: 'table',
